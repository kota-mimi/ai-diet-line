...
exports.encryptDataWithTimestamp = encryptDataWithTimestamp;
...
// タイムスタンプ指定可能な暗号化関数
async function encryptDataWithTimestamp(data, userId, timestamp) {
    try {
        const encoder = new TextEncoder();
...
// データを復号化
async function decryptData(encryptedString, userId, timestamp) {
    try {
        const decoder = new TextDecoder();
        // Base64デコード
        const combined = new Uint8Array(Array.from(atob(encryptedString), char => char.charCodeAt(0)));
        // ソルト、IV、暗号化データを分離
